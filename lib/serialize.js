...
exports.Client = ({ zev, store }, options = {}) => {
	let client = Object.defineProperties(zev, {
      ...
      parseMention: {
		...
		reply: {
			async value(jid, text, m, renderLT = false, useGroupUrl = false, options) {
...
exports.serialize = async (zev, m, store) => {
   ...
   m.message = parseMessage(m.message)
    ...
    if (m.message) {
        ...
        m.body = (m.mtype === 'conversation') ? m.message.conversation : (m.mtype == 'imageMessage') ? m.message.imageMessage.caption : (m.mtype == 'videoMessage') ? m.message.videoMessage.caption : (m.mtype == 'extendedTextMessage') ? m.message.extendedTextMessage.text : (m.mtype === 'messageContextInfo') ? (m.text) : '';
        ...
        m.timestamps = (typeof m.messageTimestamp === "number") ? m.messageTimestamp * 1000 : m.msg.timestampMs * 1000
        m.isMedia = !!m.message?.extendedTextMessage?.mimetype || !!m.message?.extendedTextMessage?.thumbnailDirectPath
      ...
      if (m.message?.extendedTextMessage?.contextInfo) {
         ...
         if (m.quoted.message) {
            ...
            m.quoted.mentions = [...(m.message?.extendedTextMessage?.contextInfo?.mentionedJid || []), ...(m.message?.extendedTextMessage?.contextInfo?.groupMentions?.map(v => v.groupJid) || [])]
            m.quoted.body = (m.mtype === 'conversation') ? m.message.conversation : (m.mtype == 'imageMessage') ? m.message.imageMessage.caption : (m.mtype == 'videoMessage') ? m.message.videoMessage.caption : (m.mtype == 'extendedTextMessage') ? m.message.extendedTextMessage.text : (m.mtype === 'messageContextInfo') ? (m.text) : '';
    ...
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? zev.sendMedia(chatId, text, 'file', '', m, { ...options }) : zev.sendText(chatId, text, m, { ...options })
